...
Code Hike aims to give structure to markdown, that can be exploited by React Components to show content tailored for the modern web.
...
Videos, slides, screenshots. Code Hike makes separating content from presentation easy. Craft the perfect style for your content using React, and reuse it across all your platforms. Did you know you can use Code Hike with Remotion?
...
For codeblocks, Code Hike provides a `highlight` function that not only applies syntax highlighting but also extract annotations from comments. The result can be passed to a `Pre` component provided by Code Hike. You can write your own React Server Components to handle those annotations, giving you again all the power of React to display code.
...
If you are building a documentation site, my current recommendation is Next.js + [Fuma Docs](https://fumadocs.vercel.app/). For examples using various frameworks, see the [examples](/docs/examples).
