---
title: Fine-grained Markdown
description: Flexible content, richer presentation
date: 2024-07-16
authors: [pomber]
---

import {
  Scrolly,
  Hover,
} from "./fine-grained/scrolly"

Markdown is great. Easy to write, easy to read, and covers most use cases for content websites.

But sometimes it feels a bit limited. Sometimes your content is not so linear, it has more structure, and you want to present it in a way that reflects that structure. You want a [richer medium](/blog/rich-content-websites) to present your content.

Let me show you what I mean, and how Code Hike can help you solve this problem.

<br />

Rendering markdown using React usually looks something like this:

<Scrolly>

## !! start

We get the content as a component. We can render the rest of the page any way you want, but the content itself is a single rigid block.

```jsx !page page.jsx -wa
import Content from "./content.md"

export function Page() {
  return (
    <div className="...">
      <SideBar />
      <Content />
    </div>
  )
}
```

```md !content content.md -wa
# Hello

Lorem ipsum dolor sit

## Step one

Amet consectetur adipiscing

## Step two

Elit sed do eiusmod

## Step three

Tempor incididunt ut labore
```

## !! frontmatter

### Frontmatter

We can convert part of the content into metadata using <Hover name="frontmatter">frontmatter</Hover>.

This is a good start, it gives us some flexibility, but the main content is still a single block of markdown.

If we want more flexibility, we need a way to break the content into smaller pieces.

```jsx !page page.jsx -wa
import {
  Content,
  frontmatter,
} from "./content.md"

const { title } = frontmatter

export function Page() {
  return (
    <div>
      <SideBar />
      <main>
        <h1>{title}</h1>
        <Content />
      </main>
    </div>
  )
}
```

```md !content content.md -wa
---
title: Hello
---

Lorem ipsum dolor sit

## Step one

Amet consectetur adipiscing

## Step two

Elit sed do eiusmod
```

## !! parseRoot

### Code Hike Blocks

To make the content really flexible, Code Hike introduces the concept of [blocks](/docs/concepts/blocks).

We define blocks by adding a <Hover name="decoration">special decoration</Hover> to headings.

Blocks can now be <Hover name="destructured">destructured</Hover> from the main content, giving us fine-grained control to render them any way we want.

```jsx !page page.jsx -wa
import Content from "./content.md"
import { parseRoot } from "codehike"

const { intro, steps } =
  parseRoot(Content)

export function Page() {
  return (
    <div>
      <SideBar />
      <main>
        {intro.children}
        {steps.map((step) => (
          <div>
            <h2>{step.title}</h2>
            {step.children}
          </div>
        ))}
      </main>
    </div>
  )
}
```

```md !content content.md -wa
## !intro Hello

Lorem ipsum dolor sit

## !!steps Step one

Amet consectetur adipiscing

## !!steps Step two

Elit sed do eiusmod
```

## !! img

We can go even further and decorate paragraphs, images, and codeblocks.

For example, we can add a <Hover name="cover">cover image</Hover> to each step.

```jsx !page page.jsx -wa
import Content from "./content.md"
import { parseRoot } from "codehike"

const { intro, steps } =
  parseRoot(Content)

export function Page() {
  return (
    <div>
      <SideBar />
      <main>
        {intro.children}
        {steps.map((step) => (
          <div>
            <h2>{step.title}</h2>
            <img src={step.cover} />
            {step.children}
          </div>
        ))}
      </main>
    </div>
  )
}
```

```md !content content.md -wa
## !intro Hello

Lorem ipsum dolor sit

## !!steps Step one

![!cover](/one.png)

Amet consectetur adipiscing

## !!steps Step two

![!cover](/two.png)
```

## !! schema

### Content Schema

We can define a schema so we get autocompletion and all the benefits of typescript in the editor.

```jsx !page page.jsx -wa
import Content from "./content.md"
import {
  parseRoot,
  Block,
  ImageBlock,
} from "codehike"
import { z } from "zod"

const Schema = Block.extend({
  intro: Block,
  steps: z.array(
    Block.extend({ cover: ImageBlock }),
  ),
})
const { intro, steps } = parseRoot(
  Content,
  Schema,
)

export function Page() {
  return <div>...</div>
}
```

```md !content content.md -wa
## !intro Hello

Lorem ipsum dolor sit

## !!steps Step one

![!cover](/one.png)

Amet consectetur adipiscing

## !!steps Step two

![!cover](/two.png)
```

## !! type-safe

### Type-safe Markdown

Adding a schema also means that now we validate that the content follows the structure we defined.

For example, since we defined that each step should have a cover image, we get a type error if we forget to add the image inside a step.

```jsx !page page.jsx -wa
import Content from "./content.md"
import {
  parseRoot,
  Block,
  ImageBlock,
} from "codehike"
import { z } from "zod"

const Schema = Block.extend({
  intro: Block,
  steps: z.array(
    Block.extend({ cover: ImageBlock }),
  ),
})
const { intro, steps } = parseRoot(
  Content,
  Schema,
)

export function Page() {
  return <div>...</div>
}
```

```md !content content.md -wa
## !intro Hello

Lorem ipsum dolor sit

## !!steps Step one

> missing cover image

Amet consectetur adipiscing

## !!steps Step two

![!cover](/two.png)
```

</Scrolly>

And that's how Code Hike gives you type-safe fine-grained markdown, making your content more flexible so you can use the whole power of React to present it in any way you want.

---

Learn more:

- [Rich content websites vs lean content websites](/blog/rich-content-websites)

<div style={{ height: "50vh" }} />
